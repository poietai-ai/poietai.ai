  ...
  return (
    <div className="flex h-screen w-screen items-center justify-center bg-zinc-950">
      <div className="flex flex-col items-center w-full max-w-2xl px-8">
        {/* Logo / brand */}
        <div className="w-12 h-12 rounded-2xl bg-violet-600 flex items-center justify-center mb-8">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-full text-xs flex items-center justify-center font-medium
                  ${i + 1 < step ? 'bg-violet-600 text-white' :
                    i + 1 === step ? 'bg-violet-600 text-white ring-2 ring-violet-400 ring-offset-2 ring-offset-zinc-950' :
                    'bg-zinc-800 text-zinc-500'}`}>
                  {i + 1 < step ? '✓' : i + 1}
                </div>
                <span className={`text-xs ${i + 1 === step ? 'text-zinc-200' : 'text-zinc-600'}`}>
                  {label}
                </span>
              </div>
