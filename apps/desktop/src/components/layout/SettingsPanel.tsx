...
export function SettingsPanel({ onClose }: Props) {
  ...
  // Lazy initializer: reads the store value once at open time; avoids an effect
  // that would overwrite in-progress edits if ghToken changes while the panel is open.
  const [draft, setDraft] = useState(() => ghToken ?? '');
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Cancel the "Saved!" flash timer if the panel unmounts before it fires.
  useEffect(() => {
    if (!saved) return;
